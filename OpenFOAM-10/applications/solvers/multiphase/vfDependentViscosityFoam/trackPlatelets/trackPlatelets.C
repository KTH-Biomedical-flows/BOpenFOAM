...
void writeCloudAsVTK(const plateletCloud & cloud, word timeName)
{
    // Create file name
    ...
    label i = 0;
    forAllConstIter(plateletCloud, cloud, iter)
    {
        const plateletParcel & p = iter();
        active.append(p.moving());
        ...
        floatScalar pRe = p.Re(p.U(), p.Uc(), p.d(), p.rhoc(), p.muc());
...
void writeCloudAsCSV(const plateletCloud & cloud, word timeName)
{
    ...
    forAllConstIter(plateletCloud, cloud, iter)
    {
        ...
        os << p.origId() << ","
           ...
           << p.Re(p.U(), p.Uc(), p.d(), p.rhoc(), p.muc()) << ","
           << p.rho() << ","
           << p.rhoc() << ","
           << p.d() << ","
...
int main(int argc, char *argv[])
{
    ...
    plateletCloud plateletCloud(cloudName, rho, U, mu, g, tau);
   ...
   if(substeps == 1) {
       forAllConstIter(List<timePair>, timeInstants, iter) {
            ...
            //runTime.setDeltaT(iter->deltaT);
            runTime.setDeltaTNoAdjust(iter->deltaT);
        ...
        forAllConstIter(List<timePair>, timeInstants, iter) {
            ...
            if(iter->timeIndex > 0) {
                runTime.setDeltaTNoAdjust(iter->deltaT);	// The false flag here prevents the runtime from adjusting the timestep we set
