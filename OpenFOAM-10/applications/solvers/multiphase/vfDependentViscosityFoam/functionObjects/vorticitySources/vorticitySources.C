...
{
    // Lookup fields
    typedef incompressible::momentumTransportModel icoTurb;
    ...
    const icoTurb & turb = mesh_.lookupObject<icoTurb>(momentumTransportModel::typeName);
    ...
    mesh_.lookupObjectRef<volVectorField>(diffusiveFluxTermName) = fvc::curl(fvc::div(turb.devSigma()));
