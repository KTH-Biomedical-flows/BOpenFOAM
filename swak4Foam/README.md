...
-   2006-2023 Bernhard F.W. Gschaider <bgschaid@hfd-research.com>
...
present these unconfigured features will not be compiled. If a
file `swakConfiguation.automatic.settings` is found then this file
is sourced before the rest of `swakConfiguraiton.automatic` is
executed. This allows fixing presets like `SWAK_PYTHON3VERSION`
...
Library with additional decomposition methods for `decomposePar`
...
Examples for the `swakDecomposition`-Library


#### crazyDecompositionPitzDaily

Decompose the case using an expression

-   **Solver:** simpleFoam
-   **Case setup:** Chose a function. For instance
    
        pyFoamPrepareCase.py . --number=8 --value="function fromWall;"


### MakeAxialMesh

Cases that demonstrate the `makeAxialMesh`-utility
...
details see [6.2.2](#orgc38e368) below). Don't work on the
...
#### Support for OpenFOAM v2112

swak4Foam compiles on this version but was not thoroughly tested


#### Support for OpenFOAM v2206

swak4Foam compiles on this version but is not yet tested


#### Support for OpenFOAM v2212

swak4Foam compiles on this version but is not yet tested


#### Compilation support for OpenFOAM 9

swak4Foam compiles on this version but is not tested

Because the code for lagrangian particles was heavily refactored
in this release everything that has to do with these was switched
off.

Almost everything in the `swakFvOptions`-library has been
disabled because in OF9 `fvOption` has been split into `fvModel`
and `fvConstraint`


#### Compilation support for OpenFOAM 10

swak4Foam compiles on this version but is not tested

Because of a refactoring of the
transport/turbulence/thermo-complex some features have been
disabled for this version

There seems to be a problem with functionPlugins


#### Compilation support for foam-extend 5.0

swak4Foam compiles on this version but is not tested (yet)


### New features


#### New function to flood fill from a point in *MeshWave*-plugins

A function `floodFillFromPoint` has been added to the function
plugin. This allows specifying a point and an expression with
blocking faces. Every cell reachable from the point is set to 1


#### Library with decomposition-method based on expressions

If

    libs (
        "libswakDecomposition.so"
    );

is added then an additional decomposition method `swak` is
available. This expects two parameters in `swakCoeffs`:

-   **processorExpression:** this should evaluate to a scalar field
    that gives for every cell the processor number it should be
    assigned to
-   **mappingMethod:** as `processorExpression` can give fractional
    results this parameter says how the floating point numbers
    should be assigned to `integer`. Possible values are:
    -   **floor:** strip the fractional part
    -   **ceil:** go to the next biggest integer
    -   **round:** round to the next integer
    -   **range:** calculate the minimum and the maximum of the
        expression and distribute the ranges evenly to the processors
        (this doesn't necessarily give balanced cell numbers)
    -   **quantile:** Calculate quantiles so that the cells are evenly
        distributed to the processors
    -   **weightedquantile:** like `quantile` but evaluates an
        expression `weightExpression` that assigns a weight to each
        processor

Before using the decomposition it is checked that a correct
CPU-number is mapped to every cell
...
#### Switch off evaluation for `groovyBCJumpAMI`

It is now possible to switch off the evaluation of this BC at
startup with

    evaluateDuringConstruction false;

This avoids problems with variables that do not exist (yet)


#### `solveTransportPDE` now accepts tensors and surface scalars as diffusion factors

For the `diffusion` entry until now only a volume scalar field
was accepted. Now it can also be a surfae scalar field or a
tensor.

This allows anisotropic diffusion to be tested/implemented


#### Automatic compilation with *Python 3* up to 3.12

The file `swakConfiguration.automatic` has been adapted to
compile with the latest Python 3 versions (if present)


#### `writeOldTimesOnSignal` more stable in parallel

For parallel runs this functionobject did not reliably write its
data. The rewrite should take care of this

It is also checked whether `FOAM_ABORT` is set to `1` because
without this on a `FatalError` the data would not be written on
...
#### `writeAdditionalFields` misses fields

This functionObject attempts to force the writing of fields by
setting their behaviour to `AUTO_WRITE`. This misses fields which
are destroyed and recreated in the meantime (with `NO_WRITE`). So
at write-time these fields are written as well. Also tries to
avoid double-writing of fields


#### Actual python-library was not used for `Python3`-integrations

The behaviour of `python-config --ldflags` changed from Python 2:
the actual library has to be speciefied explicitly. Fixed
...
#### `plane` in `simpleSearchableSurfaces` renamed to `swakPlane`

This fixes a problem in newer OpenFOAM-Versions which have a
`searchableSurface` named `plane` as well


### Infrastructure


#### Distinguish between versions of the `bear` utility

Starting with version 3 the command line options for this
differ. The compilation script tries to take this into account


#### Preset settings for `swakConfiguation.automatic`

If `swakConfiguation.automatic` is used and a file
`swakConfiguation.automatic.settings` is found then this file is
sourced before anything else is done. The main application is to
fix the *Python 3*-version to an older version even if a newer
version is found. For instance: setting the line

    export SWAK_PYTHON3VERSION=10

makes the compilation use *Python 3.10* even if 3.12 is present
