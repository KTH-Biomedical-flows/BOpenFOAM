    ...
    velocityName_("none")
{
    ...
    if(
        ...
        isA<fvMesh>(obr)
        &&
        dynamicCast<const fvMesh>(obr).dynamic()
#endif
    ) {
...
void Foam::solveTransportPDE::solve()
{
    if(active_) {
        ...
        dictionary sol=mesh
            ...
            .solutionDict()
#endif
            .subDict(fieldName_+"TransportPDE");
        ...
        for (int corr=0; corr<=nCorr; corr++) {
            ...
            autoPtr<surfaceScalarField> diffusionFieldSurface;
            autoPtr<volScalarField> diffusionFieldScalar;
            autoPtr<volTensorField> diffusionFieldTensor;

            if(driver.resultIsTyp<volScalarField>()) {
                diffusionFieldScalar.reset(
                    new volScalarField(driver.getResult<volScalarField>())
                );
                diffusionFieldScalar().dimensions().reset(diffusionDimension_);
            } else if(driver.resultIsTyp<surfaceScalarField>()) {
                diffusionFieldSurface.reset(
                    new surfaceScalarField(driver.getResult<surfaceScalarField>())
                );
                diffusionFieldSurface().dimensions().reset(diffusionDimension_);
            } else if(driver.resultIsTyp<volTensorField>()) {
                diffusionFieldTensor.reset(
                    new volTensorField(driver.getResult<volTensorField>())
                );
                diffusionFieldTensor().dimensions().reset(diffusionDimension_);
            } else{
              FatalErrorIn("Foam::solveTransportPDE::solve()")
                  << diffusionExpression_
                  << " does not evaluate to a scalar or tensor (volume) or a surface scalar" << endl
                  << exit(FatalError);
            }
	    ...
	    if(needsRhoField()) {
            ...
            fvMatrix<scalar> eq(
                ...
                -
                (
                    diffusionFieldScalar.valid()
                    ?
                    fvm::laplacian(diffusionFieldScalar(),f,"laplacian(diffusion,"+f.name()+")")
                    :
                    (
                        diffusionFieldSurface.valid()
                        ?
                        fvm::laplacian(diffusionFieldSurface(),f,"laplacian(diffusion,"+f.name()+")")
                        :
                        fvm::laplacian(diffusionFieldTensor(),f,"laplacian(diffusion,"+f.name()+")")
                    )
                )
                ==
                sourceField
	    ...
	    if(needsRhoField()) {
              ...
              eq -= fvOptions().source(rhoField(), f);
#else
