...
Contributors/Copyright:
    2012-2013, 2016-2018, 2022-2023 Bernhard F.W. Gschaider <bgschaid@hfd-research.com>
...
#ifndef FOAM_HAS_NO_SINGLEPHASE_TRANSPORT_MODEL
...
const swakKinematicTransportModel &swakTransportModelsPluginFunction::transportInternal(
    const fvMesh &reg
)
{
    static HashPtrTable<swakKinematicTransportModel> trans_;

    if(reg.foundObject<swakKinematicTransportModel>("transportProperties")) {
        if(debug) {
            Info << "swakTransportModelsPluginFunction::transportInternal: "
                << "already in memory" << endl;
        }
        // Somebody else already registered this
        return reg.lookupObject<swakKinematicTransportModel>("transportProperties");
    }
...
const swakKinematicTransportModel &swakTransportModelsPluginFunction::transport()
{
