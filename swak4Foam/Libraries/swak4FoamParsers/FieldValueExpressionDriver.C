...
Contributors/Copyright:
    2006-2018, 2023 Bernhard F.W. Gschaider <bgschaid@hfd-research.com>
...
tmp<volScalarField> FieldValueExpressionDriver::makeDistanceToField(
    ...
    FatalErrorInFunction
        << "Changed implementation of FaceCellWave in OF 10 breaks this." << nl
            << "Not yet ported to the new API and no immediate plans to do so."
            << endl
            << exit(FatalError);
#else
...
tmp<volScalarField> FieldValueExpressionDriver::makeNearDistanceField()
{
    ...
    const_cast<volScalarField::GeometricBoundaryField&>(f->boundaryField())
        ==
#ifdef FOAM_NEAR_WALL_DIST_HAS_Y_METHOD
        dist.y();
#else
        dist;
#endif
